<header class="header m-0 py-4 w-100">
    <div class="container-lg">
        <div class="row justify-content-center align-items-center">
            <div class="col-6 col-md-4">
                <div class="logo">
                    <a href="#" class="m-0 text-decoration-none text-white">Logo</a>
                </div>
            </div>

            <div class="col-6 col-md-8">
                <ul class="p-0 m-0 text-white d-flex flex-row align-items-center justify-content-end">

                    <li class="p-0 d-block ms-auto list-unstyled"><a href="#overview"
                            class="text-decoration-none text-white">Overview</a>
                    </li>

                    <li class="p-0 d-block ms-auto list-unstyled"><a href="#expect"
                            class="text-decoration-none text-white">What to expect</a>
                    </li>

                    <li class="p-0 d-block ms-auto m-0 list-unstyled"><a href="#tour"
                            class="text-decoration-none text-white">The Trip</a>
                    </li>

                    <li class="p-0 d-block ms-auto list-unstyled"><a href="#included"
                            class="text-decoration-none text-white">What's included</a>
                    </li>
                    
                    <li class="contact d-block ms-auto list-unstyled"><a href="#" class="text-decoration-none">Contact
                            us</a></li>
                </ul>
            </div>
        </div>
</header>